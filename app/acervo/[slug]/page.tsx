...
export default async function ArchiveRecordPage({ params }: Props) {
  ...
  return <main className="record-page">
    ...
    <article className="record-detail"><p className="eyebrow">{record.code} · {record.type}</p><h1>{record.title}</h1><p className="record-lead">{record.summary}</p><dl><div><dt>Data ou período</dt><dd>{record.year}</dd></div><div><dt>Local relacionado</dt><dd>{record.location}</dd></div><div><dt>Estado da informação</dt><dd>{record.status}</dd></div><div><dt>Fonte responsável</dt><dd>{record.source}</dd></div><div><dt>Última revisão</dt><dd>{record.reviewedAt}</dd></div></dl><div className="record-actions"><a className="button" href={record.sourceUrl} target="_blank" rel="noreferrer">Consultar fonte original ↗</a><a className="text-link" href="/contribua">Acrescentar informação →</a></div></article>
