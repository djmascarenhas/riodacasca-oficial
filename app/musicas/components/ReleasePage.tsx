...
export default function ReleasePage({ release, portalHref }: { release: Release; portalHref: string }) {
  ...
  return <main className="music-site release-page">
    ...
    <section className="release-listen" id="ouvir"><div className="release-listen-art"><Image src={release.image} alt="" fill sizes="(max-width: 900px) 75vw, 34vw" /></div><div><p className="section-kicker">Disponível agora</p><h2>{release.title}</h2><p className="music-by">DJ Dalma · Djalma Mascarenhas</p><div className="themes">{release.themes.map((theme) => <span key={theme}>{theme}</span>)}</div><a className="button button-dark" href={release.listenHref} target="_blank" rel="noreferrer"><span className="play" aria-hidden="true">▶</span>{release.listenLabel}</a></div></section>
